    ...
    function fetchKontenData(page = 1) {
        fetchWithAuthorization(`http://127.0.0.1:8000/api/konten?page=${page}`)
            ...
            .then(data => {
                ...
                data.forEach(konten => {
                    const getImageUrlEndpoint = `http://127.0.0.1:8000/api/get-image-url/${konten.id_konten}`;
                    fetch(getImageUrlEndpoint)
                        .then(response => response.json())
                        .then(imageData => {
                            ...
                            tr.innerHTML = `
                                ...
                                <td class="px-4 py-3 text-sm" style="padding: 4px 16px; font-size: 14px;">
                                    <img src="${imageUrl}" alt="" style="object-fit: cover; width: 40px; height: 40px;" loading="lazy" />
                                </td>
                                <td class="px-4 py-3">
